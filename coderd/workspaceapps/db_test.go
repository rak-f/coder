...
func Test_ResolveRequest(t *testing.T) {
		...
		for _, c := range cases {
			t.Run(c.name, func(t *testing.T) {
				...
				for _, app := range allApps {
					...
					assertConnLogContains(t, rw, r, connLogger, workspace, agentName, app, codersdk.ConnectionTypeWorkspaceApp, me.ID)
	...
	t.Run("AuthenticatedOtherUser", func(t *testing.T) {
		...
		for _, app := range allApps {
			...
			assertConnLogContains(t, rw, r, connLogger, workspace, agentName, app, codersdk.ConnectionTypeWorkspaceApp, secondUser.ID)
			require.Len(t, connLogger.ConnectionLogs(), 1)
		}
	})
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		for _, app := range allApps {
			...
			if app != appNamePublic {
				...
				assertConnLogContains(t, rw, r, connLogger, workspace, agentName, app, codersdk.ConnectionTypeWorkspaceApp, uuid.Nil)
				require.Len(t, connLogger.ConnectionLogs(), 1)
			} else {
				if !assert.True(t, ok) {
					dump, err := httputil.DumpResponse(w, true)
		...
		for _, c := range cases {
			t.Run(c.name, func(t *testing.T) {
				...
				if c.ok {
					...
					assertConnLogContains(t, rw, r, connLogger, workspace, agentName, token.AppSlugOrPort, codersdk.ConnectionTypeWorkspaceApp, me.ID)
					require.Len(t, connLogger.ConnectionLogs(), 1)
				} else {
					require.Nil(t, token)
					require.Len(t, connLogger.ConnectionLogs(), 0)
	...
	t.Run("TokenDoesNotMatchRequest", func(t *testing.T) {
		...
		assertConnLogContains(t, rw, r, connLogger, workspace, agentName, appNameOwner, codersdk.ConnectionTypeWorkspaceApp, me.ID)
		require.Len(t, connLogger.ConnectionLogs(), 1)
	})
	...
	t.Run("PortSubdomain", func(t *testing.T) {
		...
		assertConnLogContains(t, rw, r, connLogger, workspace, agentName, "9090", codersdk.ConnectionTypePortForwarding, me.ID)
		require.Len(t, connLogger.ConnectionLogs(), 1)
	})
	...
	t.Run("SubdomainEndsInS", func(t *testing.T) {
		...
		require.True(t, ok)
		require.Equal(t, req.AppSlugOrPort, token.AppSlugOrPort)
		assertConnLogContains(t, rw, r, connLogger, workspace, agentName, appNameEndsInS, codersdk.ConnectionTypeWorkspaceApp, me.ID)
		require.Len(t, connLogger.ConnectionLogs(), 1)
	})
	...
	t.Run("Terminal", func(t *testing.T) {
		...
		assertConnLogContains(t, rw, r, connLogger, workspace, agentName, "terminal", codersdk.ConnectionTypeWorkspaceApp, me.ID)
		require.Len(t, connLogger.ConnectionLogs(), 1)
	})
	...
	t.Run("InsufficientPermissions", func(t *testing.T) {
		...
		require.False(t, ok)
		require.Nil(t, token)
		assertConnLogContains(t, rw, r, connLogger, workspace, agentName, appNameOwner, codersdk.ConnectionTypeWorkspaceApp, secondUser.ID)
		require.Len(t, connLogger.ConnectionLogs(), 1)
	})
	...
	t.Run("RedirectSubdomainAuth", func(t *testing.T) {
		...
		assertConnLogContains(t, rw, r, connLogger, workspace, agentName, appNameOwner, codersdk.ConnectionTypeWorkspaceApp, uuid.Nil)
		require.Len(t, connLogger.ConnectionLogs(), 1)
	...
	t.Run("UnhealthyAgent", func(t *testing.T) {
		...
		defer w.Body.Close()
		require.Equal(t, http.StatusNotFound, w.StatusCode)
		assertConnLogContains(t, rw, r, connLogger, workspace, agentNameUnhealthy, appNameAgentUnhealthy, codersdk.ConnectionTypeWorkspaceApp, me.ID)
		require.Len(t, connLogger.ConnectionLogs(), 1)
...
func assertConnLogContains(t *testing.T, rr *httptest.ResponseRecorder, r *http.Request, connLogger *connectionlog.FakeConnectionLogger, workspace codersdk.Workspace, agentName string, slugOrPort string, typ codersdk.ConnectionType, userID uuid.UUID) {
	t.Helper()
