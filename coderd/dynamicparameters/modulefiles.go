...
func (r *dynamicRenderer) MissingModuleFiles() bool {
	if r.data == nil || r.data.terraformValues == nil {
		return false
	}

	return missingModuleFiles(
		r.data.terraformValues.CachedPlan,
		r.data.terraformValues.CachedModuleFiles.Valid,
	)
}
...
func missingModuleFiles(cachedPlan []byte, haveModuleFiles bool) bool {
	if haveModuleFiles {
		return false
	}

	return planUsesRemoteModules(cachedPlan)
}
