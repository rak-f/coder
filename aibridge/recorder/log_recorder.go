...
// logged.
//
// When structured is set, each record additionally produces a line in the
// format described by [InterceptionLogMarker]. apiKeyID reports the key the
// records were made under; it is only read for those lines.
...
//
// Unlike the rest of this type's logging, it runs before the record is
// delegated, so that a record dropped lower in the chain is still reported.
// coderd emits the same format for records that reach it; see the AI Gateway
// structured logging source option for which of the two is authoritative.
...
// metadataWithUserAgent returns metadata with the request's user agent added
// under [MetadataUserAgentKey], matching what coderd stores and logs.
//
// It copies rather than writing through: the record's own metadata travels on
// to coderd, which merges the user agent itself and warns when the key is
// already set.
