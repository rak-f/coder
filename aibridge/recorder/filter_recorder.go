...
// DisabledRecords selects the records a [FilterRecorder] drops. Its zero value
// disables nothing, so a [FilterRecorder] built from it delegates every call.
...
// Disabled reports whether every record type is disabled, in which case the
// [FilterRecorder] delegates nothing.
...
// [DisabledRecords], delegating the rest unchanged. It exists so that a
// deployment can keep some record types out of its database while the rest of
...
// NewFilterRecorder creates a [FilterRecorder] which drops the records selected
// by disabled and delegates the rest to wrapped. wrapped may be nil, in which
...
// [FilterRecorder], for composition by [ChainMiddleware]. As [FilterRecorder]
// documents, it belongs below the logging middleware.
